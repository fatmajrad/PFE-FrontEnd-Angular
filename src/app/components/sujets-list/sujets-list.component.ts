...
export class SujetsListComponent implements OnInit {
  ...
  constructor(private sujetService : SujetService, private modalService: NgbModal, private router:Router,private authService:AuthService) { }
  sujets : Sujet[];
  closeResult: string;
  ...
  supprimerSujet(s:Sujet) {
    this.sujetService.supprimerSujet(s.id).subscribe(() => console.log("user deleted"));
    
  }

  updateSujet(s:Sujet,id:Number){
    this.sujetService.updateSujet(s,id).subscribe(() => console.log("user modifié"));
  }

  addSujet(){
    this.sujetService.addSujet(this.newSujet).subscribe((sujet) => {});
      this.router.navigate(["/sujets-list"]);
  }
  getSujetbyNom(nom:string){
    //this.page =1;
    console.log(this.sujetService.getSujetbyNom(nom).subscribe(sujets => {
      this.sujets = sujets;
      }));
  }
